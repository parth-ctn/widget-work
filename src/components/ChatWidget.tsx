    ...
    <div style={{ position: "relative", width: "100%" }}>
      ...
      >
        {records.map((record, index) => {
          // ✅ Pre-compute recordId using buildCollectionRecordIframeDoc
          // This MUST match what was stored in featuredActionStates in toggleFeaturedCollection
          ...
          return (
            // ✅ CollectionRecordCard handles map logic internally
            // featuredRecordCollectionMap and featuredRecordUrlMap populated inside card
            <CollectionRecordCard
...
export default function ChatWidget({
  ...
  // ── Stable record IDs for impression effect dependency ───────────────────
  ...
  // ── Impression tracking — per-load local firedRef ────────────────────────
  useEffect(() => {
  ...
  const toggleFeaturedCollection = async (collectionName: string) => {
    ...
    try {
      ...
      // ✅ Get template BEFORE building initialStates
      // We need buildCollectionRecordIframeDoc to generate the SAME recordId
      // that FeaturedSlider → CollectionRecordCard will use during render
      const template = collectionTemplateMap.get(collectionName);

      // ✅ FIXED: initialStates keys = iframeDoc.recordId (NOT rec._id)
      // This prevents actionState mismatch when FeaturedSlider renders cards
      const initialStates: Record<string, RecordActionState> = {};
      // rawIdToRecordId: likesHistory API returns data keyed by raw _id
      // We need this map to correctly merge likesHistory into mergedStates
      ...
      records.forEach((rec: any, index: number) => {
        ...
        // ✅ Use buildCollectionRecordIframeDoc to get exact recordId
        // If template not yet loaded, fall back to rawId (will be fixed on next render)
        let recordId = rawId;
        if (template?.html) {
        ...
        // Populate ref maps here too (CollectionRecordCard also does this on render)
      ...
      // ✅ FIXED: Merge using rawIdToRecordId — API returns raw _id as key
      const mergedStates = { ...initialStates };
      const likesData = likesRes?.data;
      if (likesData) {
        Object.entries(likesData).forEach(([rawId, info]: [string, any]) => {
          // Map raw _id → our recordId for correct mergedStates key
                    ...
                    {featuredCollections.length > 0 && (
                      <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "16px", marginTop: "10px", overflowAnchor: "none" as any }}>
                        {featuredCollections.map((fc) => {
                          // ✅ Loading complete + no records = hide completely
                          ...
                          return (
                            <div key={fc.collectionName}>
                              {/* Clickable section label — tap to expand/collapse */}
                              <button
                                type="button"
                                onClick={(e) => {
                              ...
                              >
                                ...
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#374151" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
                                  style={{ flexShrink: 0, marginLeft: "6px", transform: isCollectionExpanded ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>
                                  <polyline points="6 9 12 15 18 9" />
                                </svg>
                              </button>
