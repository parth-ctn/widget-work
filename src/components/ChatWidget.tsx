...
import SocialIconsList from "./common/SocialIconsList";
...
const extractLikeCount = (rec: any): number =>
  rec?.like_count ?? rec?.likes ?? rec?.stats?.likes ?? rec?.stats?.ALL?.likes ?? rec?.stats?.GLOBAL?.likes ?? 0;

const extractViewCount = (rec: any): number =>
  rec?.view_count ?? rec?.views ?? rec?.stats?.impressions ?? rec?.stats?.ALL?.impressions ??
  rec?.stats?.GLOBAL?.impressions ?? rec?.stats?.views ?? rec?.stats?.ALL?.views ?? rec?.stats?.GLOBAL?.views ?? 0;

const extractShareCount = (rec: any): number =>
  rec?.share_count ?? rec?.shares ?? rec?.stats?.shares ?? rec?.stats?.ALL?.shares ?? rec?.stats?.GLOBAL?.shares ?? 0;
...
const FeaturedSlider = ({
  collectionName, records, template, featuredActionStates,
  featuredIframeRefs, featuredRecordCollectionMap, featuredRecordUrlMap,
  featuredScrollContainerRefs, onLikeToggle, onShareClick, onCardClick,
}: {
  collectionName: string;
  records: any[];
  ...
  const navBtnStyle: React.CSSProperties = {
    ...
    boxShadow: "0 1px 4px rgba(0,0,0,0.12)", color: "#5d5fef", padding: 0,
  };

  return (
    <div style={{ position: "relative", width: "100%", overflow: "hidden" }}>
      {canPrev && (
        <button type="button" onClick={() => scrollBy("prev")}
          style={{ ...navBtnStyle, left: 0 }} aria-label="Previous">
          <PrevIcon />
        </button>
      )}
      <div style={{ width: scrollTrackWidth, maxWidth: "100%", margin: "0 auto" }}>
        <div
          ref={(el) => {
          ...
          style={{
            ...
            gap: "8px", paddingBottom: "4px", width: "100%",
          ...
          {records.map((record, index) => {
            ...
            return (
              <CollectionRecordCard
                ...
                record={record} template={template} recordIndex={index}
                recordId={recordId} actionState={actionState} layout="horizontal"
      ...
      {canNext && (
        <button type="button" onClick={() => scrollBy("next")}
          style={{ ...navBtnStyle, right: 0 }} aria-label="Next">
          <NextIcon />
        </button>
      )}
    </div>
...
const ChatIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
    <g clipPath="url(#clip0_chat)">
      <mask id="mask0_chat" style={{ maskType: "luminance" }} maskUnits="userSpaceOnUse" x="0" y="0" width="15" height="15">
    ...
    <defs><clipPath id="clip0_chat"><rect width="15" height="15" fill="white" /></clipPath></defs>
  </svg>
);
...
const HistoryIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
    ...
    <defs><clipPath id="clip0_history"><rect width="15" height="15" fill="white" /></clipPath></defs>
  </svg>
);

// ── ChatWidget Main Component ─────────────────────────────────────────────────
export default function ChatWidget({
  agentId, socketUrl, userId, batchId, agentData: initialAgentData,
}: ChatWidgetProps) {
  ...
  const socialLinks = useMemo(() => {
    return (agentData.social_media ?? [])
      ...
      .filter((social) => social.type && social.link && SUPPORTED_SOCIAL_TYPES.has(social.type))
      .slice(0, 5);
  ...
  useEffect(() => {
    ...
    setFeaturedCollections(
      availableCollections.map((name) => ({
        collectionName: name, records: [], isOpen: false, isLoading: false,
      })),
    );
  ...
  useEffect(() => {
    ...
    featuredCollections.forEach((fc) => {
      if (!fc.records.length && !fc.isLoading) void toggleFeaturedCollection(fc.collectionName);
    });
  ...
  const toggleFeaturedCollection = async (collectionName: string) => {
    ...
    setFeaturedCollections((prev) =>
      prev.map((fc) => fc.collectionName === collectionName ? { ...fc, isLoading: true } : fc),
    );
    ...
    try {
      ...
      if (!token) {
        setFeaturedCollections((prev) =>
          prev.map((fc) => fc.collectionName === collectionName ? { ...fc, isLoading: false } : fc),
        );
        return;
      }
      ...
      records.forEach((rec: any, index: number) => {
        ...
        let recordId = rawId;
        if (template?.html) {
          const iframeDoc = buildCollectionRecordIframeDoc(rec, template, index);
      ...
      const likesRes = rawRecordIds.length
        ? await fetchLikesHistory({
            collection_name: collectionName, records: rawRecordIds,
            id: resolvedUserId, is_visitor: isVisitor,
          }).catch((err) => { console.error("[FEATURED_LIKES_HISTORY]", err); return null; })
        : null;
      ...
      if (likesData) {
        Object.entries(likesData).forEach(([rawId, info]: [string, any]) => {
          ...
          const serverImpressions =
            info.stats?.impressions ?? info.stats?.ALL?.impressions ??
            info.stats?.GLOBAL?.impressions ?? null;
      ...
      setFeaturedCollections((prev) =>
        prev.map((fc) => fc.collectionName === collectionName ? { ...fc, records, isLoading: false } : fc),
      );
  ...
  const {
    messages, isLoading, sendMessage, closeConnection, initialResponse,
    startNewSession, activeSessionId: activeSocketSessionId,
  } = useWebSocket(socketUrl, agentData.milvus_collection, agentId, {
    batchId, userUuid: userId, visitorId, domain: resolvedDomain,
  ...
  const collectionSessionId = useMemo(() => {
    if (activeSocketSessionId && activeSocketSessionId !== "default") return activeSocketSessionId;
    ...
    if (hasExistingConversation) {
      ...
      if (freshSocketUrl) {
        await startNewSession({
          ...
          resetState: true, connect: true, terminateOthers: true,
        });
      }
    } else {
      await startNewSession({
        sessionId: activeSocketSessionId || extractSessionIdFromUrl(socketUrl) || undefined,
        resetState: true, terminateOthers: true,
      });
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    if (activeTab === "home") await prepareNewSessionFromHome();
      ...
      await startNewSession({
        sessionId: activeSocketSessionId || extractSessionIdFromUrl(socketUrl) || undefined,
        connect: true, terminateOthers: false,
      });
    }
    sendMessage(input);
    setInput("");
  ...
  const handleSessionSelect = async (sessionId: string, loadedMessages: Message[]) => {
    ...
    await startNewSession({
      socketUrl: sessionSocketUrl ?? undefined, sessionId,
      preloadMessages: loadedMessages, connect: false,
    });
    shouldAutoScrollRef.current = true;
    setActiveTab("chat");
  };
  ...
  const tabItems: TabItem[] = [
    { key: "home", label: "Home", onClick: () => setActiveTab("home"), icon: <HomeIcon /> },
    {
      key: "chat", label: "Chat",
      onClick: () => { shouldAutoScrollRef.current = true; setActiveTab("chat"); },
      icon: <ChatIcon />,
    },
    { key: "history", label: "History", onClick: () => setActiveTab("history"), icon: <HistoryIcon /> },
  ];
  ...
  const renderChatViewToggle = () => (
    <div className="chat-action-toggle">
      <button
        ...
        aria-label="Chat view" title="Chat"
      >
      ...
      <button
        ...
        aria-label="Collection view" title="Collections"
      >
      ...
      <div className={`chat-bubble ${isOpen ? "open" : ""}`} onClick={handleChatClose}>
        {isOpen ? (
          ...
          <div className="bubble-icon">
            {/* ✅ Common: AgentAvatar */}
            <AgentAvatar src={agentData.publish_agent_profile} alt={agentData.title || "Chat"} size="bubble" />
          </div>
        )}
      </div>
              ...
              <aside className={`chat-sidebar ${isSidebarOpen ? "open" : "collapsed"}`}>
                <div className="sidebar-header">
                  <button type="button" className="sidebar-toggle-btn"
                    ...
                    aria-label="Toggle sidebar" aria-expanded={isSidebarOpen}>
                ...
                <TabNavigation tabs={tabItems} activeTab={activeTab} buttonClassName="sidebar-tab-button" wrapperClassName="sidebar-nav" />
              </aside>
            )}
            ...
            <div className="chat-main">
                ...
                <div className="chat-header">
                  <div className="agent-info">
                    <div className="agent-avatar">
                      {/* ✅ Common: AgentAvatar */}
                      <AgentAvatar src={agentData.publish_agent_profile} alt={agentData.title} size="header" />
                    </div>
                  ...
                  <div className="header-actions">
                    ...
                    <button className="close-btn" onClick={handleClose}>
                      <svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" viewBox="0 0 17 17" fill="none">
                        ...
                        <defs><clipPath id="clip0_close"><rect width="12" height="12" fill="white" transform="translate(0 8.57833) rotate(-45.6318)" /></clipPath></defs>
                      </svg>
                    </button>
                  </div>
                </div>
              ...
              {activeTab === "home" ? (
                <div className="chat-initial-screen" style={{ overflowAnchor: "none" } as any}>
                    ...
                    {isFullscreen ? (
                        ...
                        <div className="agent-profile-left">
                          <div className="agent-profile-header">
                            <div className="agent-profile-image-wrapper">
                              {/* ✅ Common: AgentAvatar */}
                              <AgentAvatar src={agentData.publish_agent_profile} alt={agentData.title} size="profile" />
                            </div>
                            <div className="agent-profile-text">
                              ...
                              <div className="agent-profile-description">
                                <p ref={descriptionRef} className="description-text"
                                  dangerouslySetInnerHTML={{ __html: agentData?.description ?? "Ask me anything! I'm here to help you find answers." }} />
                                {isTruncated && (
                                  <button className="read-more-link" onClick={() => setShowDescriptionModal(true)}>more..</button>
                                )}
                              </div>
                              {/* ✅ Common: ContactInfo (list mode) */}
                              <ContactInfo contactNumber={agentData.contact_number} email={agentData.email} mode="list" />
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="agent-profile-right">
                            {socialLinks.length > 0 && (
                              <div className="social-panel">
                                ...
                                <SocialIconsList socialLinks={socialLinks} iconBaseUrl={socialIconBaseUrl} />
                              </div>
                            )}
                            {agentData.custom_link?.link && (
                              <a href={agentData.custom_link.link} target="_blank" rel="noopener noreferrer"
                      ...
                      <>
                        ...
                        <div className="agent-action-row">
                          ...
                          <ContactInfo contactNumber={agentData.contact_number} email={agentData.email} mode="buttons" />
                        </div>
                      </>
                    )}
                    ...
                    {featuredCollections.length > 0 && (
                      <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "16px", marginTop: "10px", overflowAnchor: "none" as any }}>
                        {featuredCollections.map((fc) => {
                          if (!fc.isLoading && fc.records.length === 0) return null;
                            ...
                            <div key={fc.collectionName}>
                              <button type="button"
                                ...
                                style={{
                                  ...
                                  padding: "0 0 8px 0", cursor: "pointer", textAlign: "left",
                                }}
                                aria-expanded={isCollectionExpanded}
                              >
                                <span style={{
                                  fontSize: "11px", fontWeight: 600, color: "#374151",
                                  borderLeft: "3px solid #5d5fef", paddingLeft: "8px",
                                ...
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none"
                                  ...
                                  style={{ flexShrink: 0, marginLeft: "6px",
                                    transform: isCollectionExpanded ? "rotate(180deg)" : "rotate(0deg)",
                              ...
                              {isCollectionExpanded && (
                                <>
                                  {fc.isLoading && (
                                    ...
                                    <LoadingSpinner style={{ padding: "8px 0" }} />
                                  )}
                  ...
                  <div className="chat-input-container" style={{ width: "100%", boxSizing: "border-box" }}>
                    <div className="chat-input-field-wrap">
                      {renderChatViewToggle()}
                      <textarea ref={homeInputRef} rows={1} value={input}
                        placeholder="Type your message here..."
                        disabled={isLoading}
                        onChange={handleHomeInputChange}
                        onKeyDown={(event) => handleTextareaKeyDown(event, () => { void handleSendMessage(); })}
                        className="chat-input-field"
                      />
                    </div>
                    <button disabled={isLoading} onClick={handleSendMessage} className="send-button">
                  ...
                  {activeChatView === "collection" ? (
                    ...
                    <>
                      <div className="chat-messages" ref={messagesContainerRef}>
                        {messages.map((m, i) => {
                          const hasCollectionsSection =
                            m.type === "incoming" &&
                            (m.isCollectionLoading || m.collectionError || (m.collections && m.collections.length > 0));
                                  ...
                                  {hasCollectionsSection && (
                                    <div className="collapsible-section collection-section">
                                      <div className="section-header" onClick={() => toggleSection(i, "collections")}>
                                        ...
                                        <span className={`chevron ${isCollectionExpanded ? "open" : ""}`}>
                                          <ChevronDown size={14} />
                                        </span>
                                      </div>
                                      {isCollectionExpanded && (
                                        <div className="section-content collection-list">
                                          ...
                                          {m.isCollectionLoading && (!m.collections || m.collections.length === 0) && (
                                            <div className="collection-loading">Loading product collections...</div>
                                          )}
                                          {!m.isCollectionLoading && !m.collectionError && (!m.collections || m.collections.length === 0) && (
                                            <div className="collection-empty">No product collections found.</div>
                                          )}
                                          {m.collections?.map((collection, collectionIndex) =>
                                            renderCollectionIframe(collection, collectionIndex),
                                          )}
                                        </div>
                                      )}
                                    </div>
                                  ...
                                  {m.suggestion_questions && m.type === "incoming" && (
                                    <div className="collapsible-section">
                                      <div className="section-header" onClick={() => toggleSection(i, "suggestions")}>
                                        ...
                                        <span className={`chevron ${isSuggestionsExpanded ? "open" : ""}`}>
                                          <ChevronDown size={14} />
                                        </span>
                                      </div>
                                        ...
                                        <div className="section-content suggestions-list">
                                          {m.suggestion_questions.map((ques: string, idx: number) => (
                                            <div key={idx} className="suggestion-item"
                                              ...
                                              onKeyDown={(e) => {
                                                if (isLoading || (e.key !== "Enter" && e.key !== " ")) return;
                                                e.preventDefault();
                                            ...
                                            >
                                              ...
                                              <button type="button"
                                                ...
                                                title={copiedSuggestionKey === `${i}-suggestion-${idx}` ? "Copied!" : "Copy"}
                                              >
                                                <Copy size={14} />
                      ...
                      {showScrollToBottom && messages.length > 0 && (
                        <button type="button" className="scroll-to-bottom-btn"
                          onClick={scrollToBottom} aria-label="Scroll to bottom" title="Scroll to bottom">
                  ...
                  <div className="chat-input-container">
                    <div className="chat-input-field-wrap">
                      ...
                      <textarea ref={chatInputRef} rows={1}
                        value={activeChatView === "collection" ? collectionSearchInput : input}
                        placeholder="Type your message here..."
                        disabled={activeChatView === "chat" ? isLoading : false}
                        onChange={handleChatInputChange}
                        onKeyDown={(event) => handleTextareaKeyDown(event, () => { handleInputSubmit(); })}
                        className="chat-input-field"
                      />
                    </div>
                    <button disabled={activeChatView === "chat" && isLoading}
                      onClick={handleInputSubmit} className="send-button">
                ...
                <div className="history-tab-content">
                  {agentId && userId && (
                    <ChatHistoryList
                      batchId={agentId} userUuid={userId}
                      onSessionSelect={(sessionId, loadedMessages) => { void handleSessionSelect(sessionId, loadedMessages); }}
                      onClose={() => {}} showAll={true} agentData={agentData}
                    />
                  )}
                </div>
              ) : null}
          ...
          <div className="poweredBy">
            ...
            <a href="https://webmap.network/" target="_blank">
              <svg width="70" height="18" viewBox="0 0 70 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <defs><clipPath id="clip0_powered"><rect width="70" height="18" fill="white" /></clipPath></defs>
              </svg>
            </a>
          </div>
        </>
